...
static int shutdown_dialog_status;
...
static ssize_t get_shutdown_dialog_state(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t ret;
	ret = snprintf(buf, PAGE_SIZE, "%d\n",
			shutdown_dialog_status);
	return ret;
}
...
static ssize_t set_shutdown_dialog_state(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret = strnlen(buf, PAGE_SIZE);
	if (kstrtoint(buf, 10, &shutdown_dialog_status)) {
		pr_err("%s: kstrtoint failed\n", __func__);
		return -EPERM;
	}

	if (shutdown_dialog_status == 1) {
		amz_priv_trigger(0);
		priv_dis = true;
	} else {
		priv_dis = false;
